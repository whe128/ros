...
def cam_rgb_callback(msg):
    global node, vel_pub, vel_cmd, bridge, iLowH, iHighH, iLowS, iHighS, iLowV, iHighV
    ...
    if len(x_whole) > 0 and len(y_whole) > 0:
        ...
        vel_cmd.linear.x = (cv_image.shape[0] / 2 - y_center ) * 0.003
        vel_cmd.linear.y = 0.0
        vel_cmd.linear.z = 0.0
        vel_cmd.angular.x = 0.0
        vel_cmd.angular.y = 0.0
        vel_cmd.angular.z = (cv_image.shape[1] / 2 - x_center ) * 0.005
    ...
    print(f"robot velocity: linear.x = {vel_cmd.linear.x:.2f}, angular.z = {vel_cmd.angular.z:.2f}")
